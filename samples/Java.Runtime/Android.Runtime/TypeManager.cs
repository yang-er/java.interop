...
namespace Java.Interop {
	...
	public static partial class TypeManager {
		...
		delegate IJavaPeerable TypeCtor(ref JniObjectReference jobj, JniObjectReferenceOptions options);

		private static readonly Dictionary<Type, TypeCtor> ctors
			= new Dictionary<Type, TypeCtor>();

		internal static IJavaPeerable CreateInstance(ref JniObjectReference jobj, JniObjectReferenceOptions options, Type targetType)
		{
			if (!ctors.TryGetValue(targetType, out var ctor))
				throw new InvalidCastException("No such class registered in JI.");
			return ctor(ref jobj, options);
		}
